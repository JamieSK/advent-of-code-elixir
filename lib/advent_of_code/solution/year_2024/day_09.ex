defmodule AdventOfCode.Solution.Year2024.Day09 do
  ...
  def part2(input) do
    ...
    |> Enum.chunk_every(2)
    |> Enum.with_index()
    |> Enum.flat_map(fn {[file, space], id} ->
      [List.duplicate(id, file), List.duplicate(nil, space)]
    end)
    |> shift_lists()
    |> Enum.concat()
    |> checksum(0, 0)
  ...
  defp checksum(list, index, total),
    do: checksum(list, index + 1, total + index * (Enum.at(list, index) || 0))
  ...
  defp shift_lists(list, index) do
    case Enum.at(list, index) do
      ...
      num_list ->
        ...
        new_list =
          case Enum.find_index(list, fn space ->
                 space != [] && hd(space) == nil && length(space) >= filesize
               end) do
            nil ->
              list
            ...
            space_index when space_index < index ->
              ...
              |> then(
                &if space_length > filesize,
                  do:
                    List.insert_at(
                      &1,
                      space_index + 1,
                      List.duplicate(nil, space_length - filesize)
                    ),
                  else: &1
              )
  ...
  defp shift_and_checksum(list, compressed_index, _index, total)
       when compressed_index >= length(list),
       do: total

  defp shift_and_checksum(list, compressed_index, index, total)
       when rem(compressed_index, 2) == 0 do
    char = Enum.at(list, compressed_index)

    new_total =
      Enum.reduce(index..(index + char - 1), total, fn i, t ->
        i * div(compressed_index, 2) + t
      end)
  ...
  defp shift_and_checksum(list, compressed_index, index, total) do
    {new_list, new_index, new_total} =
      case Enum.at(list, compressed_index) do
        0 ->
          {list, index, total}
